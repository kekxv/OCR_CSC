...
// #define pthread_mutex_lock(TAG) pthread_mutex_lock(TAG);LogD("pthread_mutex_lock","pthread_mutex_lock")
// #define pthread_mutex_unlock(TAG) pthread_mutex_unlock(TAG);LogD("pthread_mutex_lock","pthread_mutex_unlock")
namespace kekxv {
    ...
    bool kekxv::VideoDevice::Check(char vid[], char pid[])
    {
            ...
            if (hr >= 0)
            {
                ...
                if (hr >= 0)
                {
                    ...
                    if (SUCCEEDED(hr))
                    {
                        ...
                        sscanf_s(str, "%*[^#]#%*[^_]_%[^&]", dev->vid, 20);
                        sscanf_s(str, "%*[^&]&%*[^_]_%[^&]", dev->pid, 20);
                    }
    ...
    int v4l2Tool::GetDevices(VideoDevice vd[], int vdLen) {
        ...
        while (len < vdLen && ((ptr = readdir(dir)) != NULL)) {
            ...
            {
                LogI("v4l2Tool", "文件%s", ptr->d_name);
                continue;
            } else if (ptr->d_type == 10) ///link file
            {
                LogI("v4l2Tool", "链接 %s", ptr->d_name);
            } else if (ptr->d_type == 4) ///dir
            {
                LogI("v4l2Tool", "目录 %s", ptr->d_name);
            } else {
                continue;
            }
            ...
            LogI("v4l2Tool", "摄像头路径 %s", (videoPath).c_str());
            ...
            LogI("v4l2Tool", "摄像头名 %s", name.c_str());

            DIR *dir1;
            struct dirent *ptr1;
            if ((dir1 = opendir((sub_dir + "/device/input").c_str())) == NULL) {
                LogE("v4l2Tool", "摄像头信息目录打开出错 %s ", (sub_dir + "/device/input").c_str());
                continue;
            }
            while ((ptr1 = readdir(dir1)) != NULL) {
                ...
                {
                    LogI("v4l2Tool", "链接 %s", ptr1->d_name);
                } else if (ptr1->d_type == 4) ///dir
                {
                    LogI("v4l2Tool", "目录 %s", ptr1->d_name);
                } else {
                    continue;
                }
                ...
                LogI("v4l2Tool", "vid : %s ;pid : %s", vid.c_str(), pid.c_str());
            }
            closedir(dir1);
            len++;
        }
    ...
    int v4l2Tool::v4l2_capabilities() {
        ...
        */
        ...
        if (xioctl(dev->fd, VIDIOC_QUERYCAP, &caps) == -1) {
            LogE("v4l2Tool", "Querying Capabilities");
            return 1;
        }
        LogD("v4l2Tool", "Driver Caps:"
                         "  Driver: \"%s\""
                         "  Card: \"%s\""
                         "  Bus: \"%s\""
                         "  Version: %d.%d"
        ...
        char c, e;
        char fourcc[5];
        LogD("v4l2Tool", "  FMT : CE Desc");
        while (0 == xioctl(dev->fd, VIDIOC_ENUM_FMT, &fmtdesc)) {
            strncpy(fourcc, (char *) &fmtdesc.pixelformat, 4);
            c = fmtdesc.flags & 1 ? 'C' : ' ';
            e = fmtdesc.flags & 2 ? 'E' : ' ';
            LogD("v4l2Tool", "  %s: %c%c %s", fourcc, c, e, fmtdesc.description);
            fmtdesc.index++;
        }
        return 0;
    }
    ...
    int v4l2Tool::v4l2_set_input() {
        ...
        */
        ...
        if (xioctl(dev->fd, VIDIOC_S_INPUT, &count) == -1) {
            LogD("v4l2Tool", "Error selecting input %d", count);
            return 1;
        }
        return 0;
    }
    ...
    int v4l2Tool::v4l2_set_pixfmt() {
        ...
        */
        ...
        if (-1 == xioctl(dev->fd, VIDIOC_S_FMT, &fmt)) {
            LogE("v4l2Tool", "Setting Pixel Format");
            return 1;
        }
        LogD("v4l2Tool", "Selected Camera Mode:"
                         "  Width: %d"
                         "  Height: %d"
                         "  PixFmt: %s",
             fmt.fmt.pix.width,
        ...
        if (dev->fmt != fmt.fmt.pix.pixelformat) {
            LogE("v4l2Tool", "Pix format not accepted");
            return 1;
        }
        LogI("v4l2Tool", "Pix format is accepted");
        return 0;
    }
    ...
    int v4l2Tool::v4l2_set_fps() {
        ...
        */
        ...
        if (xioctl(dev->fd, VIDIOC_S_PARM, &setfps) == -1) {
            LogE("v4l2Tool", "Error setting frame rate");
            return 1;
        }
    ...
    int v4l2Tool::v4l2_init_mmap() {
        ...
        */
        ...
        if (-1 == xioctl(dev->fd, VIDIOC_REQBUFS, &req)) {
            LogE("v4l2Tool", "Requesting Buffer");
            return 1;
        }
        dev->buffer = (buffer_t *) malloc(req.count * sizeof(buffer_t));
        if (!dev->buffer) {
            LogE("v4l2Tool", "Out of memory");
            dev->buffer = NULL;
            return 1;
        }
        unsigned int i;
        for (i = 0; i < req.count; i++) {
            ...
            if (xioctl(dev->fd, VIDIOC_QUERYBUF, &buf) == -1) {
                LogE("v4l2Tool", "VIDIOC_QUERYBUF");
                return 1;
            }
            dev->buffer[i].length = buf.length;
            dev->buffer[i].data = (unsigned char *) mmap(NULL, buf.length, PROT_READ | PROT_WRITE,
                                                         MAP_SHARED, dev->fd, buf.m.offset);
            if (dev->buffer[i].data == MAP_FAILED) {
                LogE("v4l2Tool", "Error mapping buffer");
                return 1;
            }
        }
        return 0;
    ...
    int v4l2Tool::stop_capturing() {
        ...
        if (xioctl(dev->fd, VIDIOC_STREAMOFF, &type) < 0) {
            LogE("v4l2Tool", "VIDIOC_STREAMOFF");
            return 1;
        }
        unsigned int i;
        for (i = 0; i < dev->n_buffers && dev->buffer != NULL; ++i) {
            if (munmap(dev->buffer[i].data, dev->buffer[i].length) < 0) {
                LogE("v4l2Tool", "munmap");
                return 1;
            }
        }
        return 0;
    ...
    int v4l2Tool::grabFrame(const char *BmpPath) {
        LogD("v4l2Tool", "be capturing..");
        int ret = pthread_mutex_lock(&testlock);
        if (ret != 0)LogD("v4l2Tool", "获取锁返回值 ：%d", ret);
        LogD("v4l2Tool", "capturing..");
...
//        LogD("v4l2Tool", "capturing..");
        ...
        if (isMirror) {
            ...
            for (int i = 0; i < width * height; i++) {
                ...
                colors[index + 2] = lrgb[index1 - 3];
                colors[index + 1] = lrgb[index1 - 2];
                colors[index + 0] = lrgb[index1 - 1];
...
#else
                colors[index1 + 0] = lrgb[index - 3];
                colors[index1 + 1] = lrgb[index - 2];
                colors[index1 + 2] = lrgb[index - 1];
#endif
                if (__j == 4) {
                    colors[index1 + 3] = 255;
                }
    ...
    int v4l2Tool::v4l2loopbackDevice() {
        ...
        */
        ...
        if (dev->fdout_devname == -1) {
            LogE("v4l2Tool", "v4l2loopback: Opening virtual video device");
            return 1;
        }
        ...
        if (xioctl(dev->fdout_devname, VIDIOC_QUERYCAP, &vid_caps) < 0) {
            LogE("v4l2Tool", "v4l2loopback: VIDIOC_QUERYCAP");
            return 1;
        }
        ...
        if (xioctl(dev->fdout_devname, VIDIOC_S_FMT, &vid_format) < 0) {
            LogE("v4l2Tool", "v4l2loopback: VIDIOC_S_FMT");
            return 1;
        }
        ...
        if (xioctl(dev->fdout_devname, VIDIOC_S_PARM, &setfps) == -1) {
            LogE("v4l2Tool", "v4l2loopback: Error setting frame rate");
            return 1;
        }
        return 0;
    }
    ...
    int v4l2Tool::GetAutoWhiteBalance() {
        ...
        if (-1 == xioctl(fd, VIDIOC_G_CTRL, &ctrl)) {
            LogE("v4l2Tool", "V4L2_CID_AUTO_WHITE_BALANCE Fail");
        }
        return ctrl.value;
    }
    ...
    int v4l2Tool::GetBrightness() {
        ...
        if (-1 == xioctl(fd, VIDIOC_G_CTRL, &ctrl)) {
            LogE("v4l2Tool", "V4L2_CID_BRIGHTNESS : 失败 ");
            return -999;
        }
        LogI("v4l2Tool", "V4L2_CID_BRIGHTNESS = 0x%x", ctrl.value);
        return ctrl.value;
    }
    ...
    int v4l2Tool::SetBrightness(int enable) {
        ...
        if (-1 == xioctl(fd, VIDIOC_S_CTRL, &ctrl)) {
            LogE("v4l2Tool", "V4L2_CID_BRIGHTNESS Fail");
            return -999;
        }
    ...
    int v4l2Tool::GetContrast() {
        ...
        if (-1 == xioctl(fd, VIDIOC_G_CTRL, &ctrl)) {
            LogE("v4l2Tool", "V4L2_CID_CONTRAST : 失败 ");
            return -999;
        }
        LogI("v4l2Tool", "V4L2_CID_CONTRAST = 0x%x", ctrl.value);
        return ctrl.value;
    }
    ...
    int v4l2Tool::SetContrast(int enable) {
        ...
        ctrl.value = enable; // queryctrl.default_value;
        if (-1 == xioctl(fd, VIDIOC_S_CTRL, &ctrl)) {
            LogE("v4l2Tool", "V4L2_CID_CONTRAST Fail");
            return -999;
        }
        return GetBrightness() == enable;
    }
    ...
    int v4l2Tool::Open(const char *devname) {
        ...
        Close();
        LogD("v4l2Tool", "OpenVideo : %s", devname);
        running = 0;
        ...
        // print args
        LogD("v4l2Tool", "Required width: %d", dev->width);
        LogD("v4l2Tool", "Required height: %d", dev->height);
        LogD("v4l2Tool", "Required FPS: %d", dev->fps);
        LogD("v4l2Tool", "input device: %s", dev->in_devname);
        LogD("v4l2Tool", "output device: %s", dev->out_devname);

        // dev->fd = open(dev->in_devname, O_RDWR | O_NONBLOCK, 0);
        dev->fd = open(dev->in_devname, O_RDWR, 0);
        LogI("v4l2Tool", "video fd : %d ", dev->fd);
        // if (dev->fd <=0)
        // {
        //     LogE("v4l2Tool", "Opening video device");
        //     close(dev->fd);
        //     dev->fd = open(dev->in_devname, O_RDWR | O_NONBLOCK, 0);
    ...
    void *v4l2Tool::ReadVideo(void *arg) {
        ...
        while (!that->isClose) {
            ...
            if (-1 == r) {
                LogE("v4l2Tool", "Waiting for Frame");
            ...
            if (that->xioctl(that->dev->fd, VIDIOC_DQBUF, &buf) < 0) {
                ...
                pthread_mutex_unlock(&that->testlock);
                LogE("v4l2Tool", "VIDIOC_DQBUF continue");
                continue;
            }
            that->VIDIOC_DQBUF_Fail_Flag = false;
            ...
            pthread_mutex_unlock(&that->testlock);
            // LogI("v4l2Tool", "VIDIOC_QBUF once");
        }
        that->running = 0;
        ...
        if (that->frame_buffer != NULL) {
            int ret = pthread_mutex_lock(&that->testlock);
            if (ret != 0)LogD("v4l2Tool", "获取锁返回值 ：%d", ret);
            LogD("vel2Tool", "free frame_buffer");
            if (that->frame_buffer != NULL)
                free(that->frame_buffer);
            LogD("vel2Tool", "freeEnd frame_buffer");
            that->frame_buffer = NULL;
            pthread_mutex_unlock(&that->testlock);
        }

        LogD("v4l2Tool", "VIDIOC_QBUF is exit");
        pthread_exit(NULL);
        return NULL;
    }
    ...
    void v4l2Tool::Close() {
        ...
        // {
        //     // LogD("free","file : %s , line : %d",__FILE__,__LINE__);
        //     free(frame_buffer);
        //     // LogD("freeE","file : %s , line : %d",__FILE__,__LINE__);
        //     frame_buffer = NULL;
        // }
